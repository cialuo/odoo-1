...
class lty_cloud_server_config(models.Model):
    _name = 'lty.cloud.server.config'
    _inherit = 'res.config.settings'

    @api.model_cr
    def get_latest_config(self):
        self._cr.execute('select name,port ,access_key from lty_cloud_server_config where id = (select max(id)  from lty_cloud_server_config)')
