...
class Product(models.Model):
    ...
    def _search_default_code(self, operator, value):
        ...
        """
        ids = []
        self._cr.execute("""SELECT p.id FROM product_product p JOIN product_category c ON (c.id=p.categ_id) WHERE (c.code || p.inter_code) ILIKE %s OR p.inter_code ILIKE %s """, ("%%%s%%" % value,"%%%s%%" % value))
