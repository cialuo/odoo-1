...
class StockPicking(models.Model):
    ...
    def action_confirm(self):
        for order in self:
            ...
            if type in [u'退料'] and self.warranty_order_id:
                location_id = self.env.ref('stock_picking_types.stock_location_ullage').id  # 维修(生产)虚位
                self.write({
                    'location_id': location_id
                })

            elif type in [u'发料', u'领料'] and self.warranty_order_id:
                # if self.warranty_order_id.materials_control:
                '''
                ...
                '''
                if type == u'领料':
                    res_get = self.env['stock.picking'].search([('warranty_order_id', '=', self.warranty_order_id.id),
                                                            ('state', 'not in', ['cancel']),
                                                            ('picking_type_id.name', 'in', [u'发料', u'领料']),
                                                            ])
                    res_back = self.env['stock.picking'].search([('warranty_order_id', '=', self.warranty_order_id.id),
                                                                ('state', 'in', ['done']),
                ...
                if move_lines:
                    picking = self.env['stock.picking'].create({
                        'origin': self.warranty_order_id.name,
                        'location_id': location_id,
